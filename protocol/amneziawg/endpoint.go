...
func RegisterEndpoint(registry *endpoint.Registry) {
	endpoint.Register[option.AmneziaWGEndpointOptions](registry, C.TypeAmneziaWG, NewEndpoint)
}
...
func NewEndpoint(ctx context.Context, router adapter.Router, logger log.ContextLogger, tag string, options option.AmneziaWGEndpointOptions) (adapter.Endpoint, error) {
	ep := &Endpoint{
		Adapter:        endpoint.NewAdapterWithDialerOptions(C.TypeAmneziaWG, tag, []string{N.NetworkTCP, N.NetworkUDP}, options.DialerOptions),
		ctx:            ctx,
		router:         router,
	...
	outboundDialer, err := dialer.NewWithOptions(dialer.Options{
		...
		RemoteIsDomain: common.Any(options.Peers, func(it option.AmneziaWGPeer) bool {
	...
	awgEndpoint, err := amneziawg.NewEndpoint(amneziawg.EndpointOptions{
		...
		Peers: common.Map(options.Peers, func(it option.AmneziaWGPeer) amneziawg.PeerOptions {
